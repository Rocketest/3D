...
def fakeline(x1, y1, z1, x2, y2, z2):
    ...
    return (Z1 > near_plane) and (Z2 > near_plane)
def line(x1, y1, z1, x2, y2, z2, size):
    global X1, Y1, Z1, X2, Y2, Z2
    setpoint1((x1 - cam_x), (y1 - cam_y), (z1 - cam_z))
    setpoint2((x2 - cam_x), (y2 - cam_y), (z2 - cam_z))
    setpoint1((Z1 * cam_dir_y_sin) + (X1 * cam_dir_y_cos), Y1, (Z1 * cam_dir_y_cos) - (X1 * cam_dir_y_sin))
...
def drawtriangle(dist, x1, y1, z1, x2, y2, z2, x3, y3, z3, color, pcolor=""):
    if fakeline(x1, y1, z1, x2, y2, z2) and fakeline(x2, y2, z2, x3, y3, z3) and fakeline(x3, y3, z3, x1, y1, z1):
        ...
        line(x2, y2, z2, x3, y3, z3, 2)
        line(x3, y3, z3, x1, y1, z1, 2)
        end_fill()
    else:
        color = hex_to_rgb(color)
        pcolor = hex_to_rgb(pcolor)
        ...
        pcolor[2] = int(pcolor[2] * factor)
        pencolor(color)
        d = float((((x1 - x2) ** 2) + ((y1 - y2) ** 2) + ((z1 - z2) ** 2)) ** 0.5)
        step = [((x1 - x2) / d) * 2, ((y1 - y2) / d) * 2, ((z1 - z2) / d) * 2]
        stepx = ((x1 - x2) / d) * 2
        stepy = ((y1 - y2) / d) * 2
        stepz = ((z1 - z2) / d) * 2
        d = d / 2
        ...
        if pcolor != "":
            pencolor(pcolor)
        line(x1, y1, z1, x2, y2, z2, 10)
        line(x2, y2, z2, x3, y3, z3, 10)
        line(x3, y3, z3, x1, y1, z1, 10)
